...
import AssetLoader from '../utils/DomAssetLoader'
//import Item from './Item'
import Section from './DomSection'
import Konami from 'konami'

import { CSS3DRenderer, CSS3DObject } from '../../customThree/THREECSS3DOM'
...
export default class Timeline {
    ...
    createTimeline() {
        ...
        let string = `<div><h1>This is an H1 Element.</h1>
        ...
        for ( let i = 0; i < 8; i++ ) {
            let month = i;
            if (i === 0) month = 'intro';
            else if (i === 6) month = 'dec'
            else if (i === 7) month = 'end';
            else month = 'mon'+String(month);
            
            this.sections[ month ] = this.createCSS3DObject(string.replace('H1',month));

            //this.section[ month ].position.set( 0, 0, 0 )
            ...
            if( month === 'end' ) posOffset = 2700


            //console.log((pointMonth+800) * (i+1) - posOffset)
    ...
    moveToStart() {
        ...
        TweenMax.to( this.camera.position, 2, {
            ...
            onComplete: () => {
                ...
                sectionKeys.forEach((month) => {
                    this.sections[month].position.y = 0
                });
            }
        })
    ...
    openContact( e ) {
        ...
        TweenMax.to( this.camera.position, 2, {
            y: newY,//this.contactSection.position.y * this.scene.scale.y,
            ease: 'Expo.easeInOut',
            onComplete: () => {
                this.timeline.visible = false
                if (this.c.isMobile) this.contactSection.position.y = 0
    ...
    closeContact() {
        ...
        //this.contactSection.position.y = -2000
        TweenMax.to( this.camera.position, 2, {
            y: 0,
            ease: 'Expo.easeInOut',
            onComplete: () => {
